...
const WsResponse = () => {
    ...
    useEffect(() => {
        if (ws_res) {
            if (ws_res[open_api_now]) {
                setMsgList(ws_res[open_api_now].result);
            } else {
                setMsgList([]);
            }
        }
    ...
    useEffect(() => {
        if (ws_res[open_api_now] && Object.entries(ws_res[open_api_now] || {}).length > 0) {

            let list = (ws_res[open_api_now].result || []).filter(item => (tabId === '0' ? item.request_body : item.response_body).includes(keyword));

            setMsgList(list);
        }
    }, [keyword, tabId, ws_res, open_api_now]);
    ...
    const tabList = [
        {
            ...
            content: <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <div className="ws-manage-response-header">
                    <div className="left">
                    ...
                    {
                        ws_res[open_api_now] && ws_res[open_api_now].status === 'running' ?
                            <div className="connect-status">{t('apis.tcp.connectSuccess')}</div>
                            : <></>
                    }
                </div>
                <div className="ws-manage-response-container">
                    {
    ...
    return (
        <div className="ws-manage-response">
